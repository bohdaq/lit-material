  ...
  override render() {
    return html`
      ...
      <p>
        ...
        links in the top bar, or click below — every navigation is a client-side route change, not a full
        page reload (open devtools' Network tab and watch).
      </p>
      <p>
        The counter on <code>/counter</code> lives in a <code>@lit-material/store</code> instance shared by
        this whole app: navigate away and back and its value is still there. The current color scheme
