...
/**
 * A single destination inside a `lit-material-navigation-bar` — an icon in a
 * pill-shaped active indicator, with a small label below it, following the
 * MD3 navigation bar item spec. Visually and structurally almost identical
 * to `lit-material-navigation-rail-item` (same icon-pill + label-below
 * layout, badge slot, ripple/focus-ring scoped to the indicator) — the
 * difference is this item shares the bar's width equally with its siblings
 * (`flex: 1`) instead of filling a narrow vertical rail's width, and its
 * indicator is 64px wide (the MD3 bar spec) rather than the rail's 56px.
 *
 * Renders as a real `<button>` by default, or an `<a>` when `href` is set.
 * `selected` is normally managed by the parent bar (see its docs), not set
 * directly.
 *
 * @element lit-material-navigation-bar-item
 *
 * @slot - The label text.
 * @slot icon - The icon.
 ...
 */
@customElement("lit-material-navigation-bar-item")
export class LitMaterialNavigationBarItem extends LitElement {
...
declare global {
  interface HTMLElementTagNameMap {
    "lit-material-navigation-bar-item": LitMaterialNavigationBarItem;
  }
}
